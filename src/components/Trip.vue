...
async function delTrip(){
    if(!confirm("Are you sure you want to delete this precious trip?")) return;
    const token = userStore.token;
    if(!token) return;

    let url = new URL(server_url+"/trip/" + props.data._id);
...
<div class="tripEntry">
    ...
    <div class="details">
        ...
        <div v-if="campground">
            <label class="info-label" for="">Campground: </label>
            <span>{{ campground.name }}</span>
        </div>
        <div v-if="activites">
            <label class="info-label" for="">Activities: </label>
            <li v-for="a in activites">{{ a.title }}</li>
        </div>
        <br>
        <div class="two-col">
            <div>
    ...
    <div class="btn-cont">
        <button class="del-trip" @click="delTrip()">Delete Trip</button>
